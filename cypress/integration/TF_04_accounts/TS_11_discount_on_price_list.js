context('Discount Check on Price List Rate', () => {
	...
	it('Discount on price list rate applied as percentage in SO', () => {
	...
	it('Discount on price list rate applied as amount in SI', () => {
		cy.new_doc('Sales Invoice');
		cy.location('pathname').should('include', '/app/sales-invoice/new-sales-invoice');
		...
		cy.wait(600);
		cy.open_section('Discount and Margin');
		cy.get_input('discount_amount').click();
		cy.wait(500);
		cy.open_section('Discount and Margin');
		cy.set_input('discount_amount', 4400);
		cy.get_input('discount_amount').blur();

		cy.findByText('Discount and Margin').click({force: true});
		cy.open_section('Discount and Margin');
		cy.get_input('discount_amount').should('have.value', "4,400.00");
		cy.close_grid_edit_modal();

		cy.get_read_only('total_qty').should('contain', "1");
		cy.get_read_only('total').should('contain', "₹ 1,05,600.00");
		cy.get_read_only('grand_total').should('contain', "₹ 1,05,600.00");
		cy.get_read_only('rounded_total').should('contain', "₹ 1,05,600.00");
